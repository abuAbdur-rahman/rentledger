...
export async function GET(req: NextRequest) {
  ...
  if (statusFilter !== "all") {
    query = query.eq("status", statusFilter as "pending" | "active" | "rejected" | "terminated")
  }

  const { data: tenancies, count, error } = await query
  ...
  for (const p of payments) {
    balanceMap.set(p.tenancy_id, (balanceMap.get(p.tenancy_id) ?? 0) + (p.amount ?? 0))
  }
  ...
  const items: TenantItem[] = (tenancies ?? []).map((t) => ({
    ...
    status: (t.status === "pending" || t.status === "active" || t.status === "rejected" || t.status === "terminated") ? t.status : "pending",
