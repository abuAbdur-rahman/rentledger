...
export async function GET(req: NextRequest) {
  ...
  const items: PropertyItem[] = (properties ?? []).map((p: any) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const activeTenancies = allTenancies.filter((t: any) => t.status === 'active')
    
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const pending = allTenancies.filter((t: any) => {
      if (t.status !== 'active') return false
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const hasPendingPayment = t.payments?.some((pay: any) => pay.status === "pending")
    ...
    const overdue = allTenancies.filter((t: any) => {
      if (t.status !== 'active') return false
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const hasPayment = t.payments?.some((pay: any) => pay.status !== "paid" && pay.status !== "verified")
...
export async function POST(req: NextRequest) {
  ...
  if (!unitsCount || unitsCount < 1) return NextResponse.json({ error: "Number of units is required." }, { status: 400 })
  if (!rentAmount || rentAmount < 0) return NextResponse.json({ error: "Rent amount is required." }, { status: 400 })

  // Create the property
  const { data: property, error } = await supabase
    .from("properties")
    .insert({ landlord_id: userId, name: name.trim(), address: address?.trim() ?? null })
