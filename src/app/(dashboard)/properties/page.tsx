...
import { Plus, Search, Building2, RefreshCw, SlidersHorizontal } from "lucide-react"
...
export default function PropertiesPage() {
  ...
  const filtered = useMemo(() => {
    let list = properties
    if (search.trim()) list = list.filter(p => p.name.toLowerCase().includes(search.toLowerCase()) || p.address.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <>
      ...
      <div className="px-4 py-4 lg:px-8 lg:py-8 max-w-[1200px] mx-auto w-full">
        <PageHeader title="Properties" subtitle={`${pagination.total} propert${pagination.total !== 1 ? "ies" : "y"}`} user={headerUser}
          action={
        ...
        <div className="flex gap-3 mb-6">
          ...
          <Select value={filter} onValueChange={setFilter}>
            <SelectTrigger className="w-[140px] h-11 rounded-[8px] border-gray-200 text-sm bg-white gap-2">
              <SlidersHorizontal className="w-4 h-4 text-gray-400" /><SelectValue />
            </SelectTrigger>
        ...
        {!loading && error && (
          <div className="space-y-4">
            <Alert variant="destructive" className="border-red-200 bg-red-50 rounded-[12px]">
              <AlertTitle className="font-semibold">Failed to load properties</AlertTitle>
              <AlertDescription className="text-sm mt-1">{error}</AlertDescription>
            </Alert>
            <Button onClick={() => fetchProperties(pagination.page)} variant="outline" className="rounded-[8px] border-gray-200 gap-2">
        ...
        {!loading && !error && properties.length === 0 && (
          <div className="flex flex-col items-center justify-center min-h-[45vh] text-center">
            ...
            <h3 className="text-lg font-black tracking-[-0.02em] text-gray-900 mb-2">No properties yet</h3>
        ...
        {!loading && !error && filtered.length > 0 && (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {filtered.map(p => <PropertyCard key={p.id} {...p} />)}
            </div>
            <Pagination
      ...
      <AddPropertyDialog open={dialogOpen} onOpenChange={setDialogOpen} onSuccess={fetchProperties} />
    </>
  )
}
